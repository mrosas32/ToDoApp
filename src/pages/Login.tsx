...
export default function Login() {
  ...
  const handleAction = async () => {
    ...
    if (isRegistering) {
        const success = await register(usuario, clave);
        if (success) {
            setToast('¡Cuenta creada! Ya puedes ingresar.');
            setIsRegistering(false);
        } else {
            setToast('El usuario ya existe. Intenta con otro nombre.');
        }
    } else {
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding ion-text-center">
        ...
        <IonToast
          ...
          position="bottom"
          color={toast.includes('Bienvenido') || toast.includes('creada') ? 'success' : 'danger'}
        />
      </IonContent>
    </IonPage>
  );
